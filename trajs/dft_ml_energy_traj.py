...
if __name__ == '__main__':
    ...
    print("Sampling the trajectories...")
    num_of_selected_trajs = 15

    # if lable_list not all None
    if has_labels:
        print("Stratified sampling based on labels...")

        # One label per trajectory — use the first non-None label as representative
        traj_labels = []
        for labels in label_list:
            valid = [l for l in labels if l is not None]
            traj_labels.append(valid[0] if valid else "unknown")

        traj_labels = np.array(traj_labels)
        print("Trajectory labels example:", traj_labels)

        # Stratified sampling — proportional to label distribution
        sss = StratifiedShuffleSplit(
            n_splits=1,
            train_size=min(num_of_selected_trajs, len(traj_labels)),
            random_state=42
        )

        try:
            idx, _ = next(sss.split(np.zeros(len(traj_labels)), traj_labels))
            selected_indices = idx.tolist()
        except ValueError as e:
            print(f"⚠️ Stratified sampling failed ({e}), falling back to random sampling.")
            selected_indices = random.sample(range(len(a)), num_of_selected_trajs)
    else:
        print("No valid labels found. Using random sampling.")
        selected_indices = random.sample(range(len(a)), num_of_selected_trajs)
