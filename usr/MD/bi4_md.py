...
def generate_bi4_bi7(bi4, bi7, n,
                     ...
                     tol=0.2):
    """
    Generate n structures of type Bi4 + Bi7.
    """
    bi4 = center(np.asarray(bi4))
    bi7 = center(np.asarray(bi7))

    samples = []

    for _ in tqdm(range(n), desc="Generating Bi4+Bi7"):
        success = False

        for _attempt in range(max_attempts_per_sample):
            placed = place_fragment(
                bi7, bi4,
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Bi4-based starting structures and run MD in one script."
    )
    ...
    parser.add_argument("--number", type=int, required=True,
                        help="Number of Bi4+Bi7 structures to generate.")
    ...
    parser.add_argument("--prefix_bi4", type=str, default="Bi4-2")
    ...
    parser.add_argument("--out_dir", type=str, default="bi4")

    parser.add_argument("--core_frag_min_atomic_distance", type=float, default=3.0,
                        help="Min allowed atom-atom distance between Bi7 and one Bi4 fragment.")
    ...
    bi4_path = Path(args.building_block_dir) / f"{args.prefix_bi4}_optimized.xyz"
    bi7_path = Path(args.building_block_dir) / f"{args.prefix_bi7}_optimized.xyz"

    bi4 = read_xyz(str(bi4_path))[0]
    ...
    coords = np.array(samples_bi4, dtype=float)
    labels = np.array(["synthesis_bi4"] * len(coords))

    print("Generated dataset:", coords.shape)

    # Save generated structures for later reuse/debugging
    dataset_prefix = f"bi4_td{args.target_dist:g}_n{args.number}"
