...
class Generate_TrajsBatch:
    ...
    def get_predicted_energy_and_forces(self, data_list, step = None, evaluate_timing_file=None):
        ...
        t_force_rearrange = time.time()
        if step is not None and step % 1000 == 0:
            evaluate_timing_file.write(f"{step},{t_convert - t0:.4f},{t_to_tensor - t_convert:.4f},{t_pred - t_to_tensor:.4f},{t_force_rearrange - t_pred:.4f}\n")
            evaluate_timing_file.flush()
