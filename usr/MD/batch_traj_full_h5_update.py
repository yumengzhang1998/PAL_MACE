...
class Generate_TrajsBatch:
    def __init__(
        ...
        # --------- BUILD CACHE ONCE ----------
        print("📦 Building data object cache (once)...")
        self.cache = convert_to_data_object(self.data_batch)
    ...
    def _max_pair_distance(coords_angstrom: np.ndarray) -> float:
        ...
        if coords_angstrom.shape[0] < 2:
            return 0.0
        return float(pdist(coords_angstrom).max()), float(pdist(coords_angstrom).min())
    ...
    def set_up_simulations(self, data_batch):
        ...
        for idx, data in enumerate(tqdm(data_batch, desc="Setting up simulations")):
            try:
                ...
                continue
        return simulators
    def init_h5(self, h5_path, steps, n_atoms, n_trajs, labels=None, compression="gzip"):
    ...
    def run_mixed_skip_bad(self, steps, h5_path, label_batch=None, final_path=None):
        ...
        log_buffer = []      # ← add this
        LOG_BUFFER_SIZE = 10 # ← flush every 50 explosions
        ...
        try:
            for step in tqdm(range(steps), desc="MD (mixed_skip_bad)", mininterval=5.0):
                ...
                t_openmm_1 = time.time()

                # update frames for next ML call (only active) & Explosion check 
                for i in range(n_trajs):
                    if active[i]:
                        frames[i][0] = coords_ang[i]
                        last_good_coords[i] = coords_ang[i]#.copy()
                        max_dist, min_dist = self._max_pair_distance(coords_ang[i])
                        if max_dist > self.r_max or min_dist < 1.5:
                            label = label_batch[i] if label_batch else "None"
                            print(f"💥 Traj {i} ({label}) exploded at step {step} (max={max_dist:.2f}, min = {min_dist:.2f})")

                            # log_file.write(f"{i},{label},{step},{max_dist:.6f}\n")
                            log_buffer.append(f"{i},{label},{step},{max_dist:.6f}\n")
                            # log_file.flush()
                ...
                if step % self.save_stride == 0:
                    ...
                    if write_step % 200 == 0:
                        h5f.flush()
                        t_save_1 = time.time()
                        print(f"save {step} | h5write {t_save_1-t_save_0:.3f}s", flush=True)
                if step % 1000 == 0:
                    print("mem alloc/reserved (MB):", torch.cuda.memory_allocated() / 1e6, "/", torch.cuda.memory_reserved() / 1e6, flush=True)
                    print(
                        f"step {step} | mace {t_mace_1-t_mace_0:.3f}s | openmm+sync {t_openmm_1-t_openmm_0:.3f}s",
                        flush=True
                    )
    ...
    def get_predicted_energy_and_forces(self, data_list):
        """
        Reuse cached Data objects.
        Only update positions.
        Do NOT rebuild graph structure.
        """

        # data_list order is guaranteed same as self.cache order
        # Only update positions

        for data_obj, frame in zip(self.cache, data_list):

            coords = frame[0]  # numpy array (Å)

            # Create fresh leaf tensor each step (prevents graph chaining)
            coords_tensor = torch.from_numpy(coords).to(self.device)

            data_obj.pos.copy_(coords_tensor)
        # ---- Run evaluation exactly as before ----
        y_pred, force_pred, _, _ = evaluate(
            model=self.model,
            eval_dataset=self.cache,
            batch_size=128,
        ...
        num_data = len(self.cache)

        force_pred = torch.stack(force_pred).cpu().numpy()
