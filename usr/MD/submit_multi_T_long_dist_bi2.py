...
script_name = "bi2_md.py"
log_dir = Path("slurm_logs/bi2")
log_dir.mkdir(parents=True, exist_ok=True)
slurm_dir = Path("slurm_jobs/bi2")
...
#SBATCH --gres=gpu:1
#SBATCH --output=slurm_logs/bi2/bi2_{T}K_%j.out
#SBATCH --error=slurm_logs/bi2/bi2_{T}K_%j.err
...
save_results() {{
  ...
  SRC="$JOB_TMP/results"
  DEST="/lsdf/kit/int/hv3694/results/PAL_MACE/trajs/bi2/run_${{SLURM_JOB_ID}}_{T}K"
...
python {script_name} \\
  ...
  --steps {steps} \\
  --T {T}.0 \\
  --out_dir bi2/${{SLURM_JOB_ID}}_{T}K
...
for T in temperatures:
    slurm_path = slurm_dir / f"run_bi2_{T}K.slurm"
