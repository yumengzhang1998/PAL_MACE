...
def get_res(sample, test_data, prefix, batch_size):
    model = torch.load(f"full_data_charge_embed/bi0_logs/sample_{sample}/bi0.model")  
    model.eval()
...
prefix = 'bi4-6'
df = pd.read_csv(f"full_data_charge_embed/bi0_logs/bi0.csv")
...
plt.savefig(f'full_data_charge_embed/bi0_logs/{prefix}_std_hist.png')
