...
class UserGene(object):
    ...
    def __init__(self, rank, result_dir):
        ...
        self.n_state = int(self.job_keywords["molecule"]["ci"])    # number of electronic states for simulation
        ...
        with open(generator_setting.init_cond_path, 'rb') as fh:
    ...
    def generate_new_data(self, data_to_gene):
        ...
        if self.traj_count >= self.generator_setting.max_traj_per_process:
    ...
    def save_progress(self, stop_run):
        ...
        save_thread = threading.Thread(target=md_tools.save_pickle, name=f"save_traj_{rank}",\
                                    args=(self.traj_data, self.step_keys, self.traj_keys,\
                                          [self.save_path,None], mode, self.aimd.traj.coord,\
    ...
    def _init_traj(self):
        ...
        self.traj_dir = os.path.join(self.result_dir, f"traj_{self.rank}")
