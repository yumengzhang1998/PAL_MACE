...
import torch
import sys
sys.path.append("../usr/initial_pyg/")
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            ...
            if raw_path.endswith(".csv"):
                ...
                elements_number = data["atoms"].values
                elements_number = [literal_eval(e) for e in elements_number]

                elements = [[periodictable.elements[ei]for ei in e] for e in elements_number]
...
def run(args: argparse.Namespace, train, val, test = None, latent = False, charge_penalty = None) -> None:
    ...
    true_energy = torch.cat([t.view(-1) for t in true_energy], dim=0).cpu().tolist()
    ...
    plot_true_vs_predicted(true_forces.flatten().cpu().tolist(), np.array(forces).flatten().tolist(), args.results_dir ,'Forces')
    ...
    plot_true_vs_predicted(true_forces.flatten().cpu().tolist(), np.array(forces).flatten().tolist(), args.results_dir ,'Train_Forces')
...
def make_data(prefix, config_path, num_atom, charge,res_dir="nocharge", latent = False, charge_penalty = None, clean = False, i = 0):
    config = ConfigLoader(config_path)
    args_dict = config['args_dict']
    args = build_default_arg_parser_dict(args_dict)   
    print(args.swa)

    results_dir = f"{res_dir}/{prefix}_logs_ensamble{i}/" if clean == False else f"{res_dir}/{prefix}_clean_logs/"
    ...
    val_df.to_csv(f'{results_dir}/{prefix}.csv', index=False, header=['atoms', 'coordinates', 'total_energy', 'forces', 'charge', 'source'])
    ...
    result_of_sample_dir = f"{results_dir}/"

    sample_file = f"{results_dir}/train_val.pkl"


    with open(sample_file, "wb") as f:
        pickle.dump((train_split, val), f)
    ...
    } for d in train_split]).to_csv(f"{results_dir}/train.csv", index=False)
    ...
    pd.DataFrame([{
        ...
        'charge': d.charge,
        'source': d.source if hasattr(d, 'source') else None
    } for d in val]).to_csv(f"{results_dir}/val.csv", index=False)

    train_energy = [d.y for d in train_split]

    train_energy = torch.cat([t.view(-1) for t in train_energy], dim=0).cpu().tolist()


    val_energy = [d.y for d in val]
    val_energy = torch.cat([t.view(-1) for t in val_energy], dim=0).cpu().tolist()
    ...
    run(args, train_split, val, latent=latent, charge_penalty=charge_penalty)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Single Training Run (no bootstrapping)")
    ...
    parser.add_argument("--results_dir", type=str, default="ensamble_results")
    ...
    # if "sample" contains in args.charge, charge = -3, else int(args.charge)
    # Determine charge automatically
    if "sample" in str(args.charge).lower():
        charge = -3
    else:
    ...
    print(type(clean))
    for i in range(2):
        make_data(prefix, config_path, num_atom, charge, res_dir=res_dir, latent=False, charge_penalty=None, clean = clean, i=i)
